...
class GPTConfig:
    ...
    vocab_size: int = 50257 #number of tokens:50 000 BPE merges + 256 bytes tokens + 1 EOF token
    ...
    class GPT(nn.Module):
        ...
        def from_pretrained(cls, model_type):
            ...
            assert model_type in {'gpt2', 'gpt2-medium', 'gpt-large', 'gpt2-xl'}
            ...
            sd = model.state_dict()
            sd_keys = sd.keys()
            sd_keys = [k for k in sd_keys if not k.endswith('.attn.bias')] #discard this mask/buffer key

            #init a hugging face/transformers model
            model_df = GPTLMHEadModel.from_pretrained(model_type)
            ...
            #basically the openai checkpoints use a "conv1D" module, but we only want touse a vanilla Linear
            #This means that we have to tranpose these weights when we import them
